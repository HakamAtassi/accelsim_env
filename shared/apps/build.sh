...
CONFIG=$1
GPGPU_CFG=/root/Repos/shared/accel-sim-framework/gpu-simulator/gpgpu-sim/configs/tested-cfgs/SM86_RTX3070/gpgpusim_$1.config
...
#NUM_TRIALS=5   # or however many trials you want
...
#for wl in "${WORKLOADS[@]}"; do
  ...
  #echo "" > "${wl}.out_$CONFIG" # clear file 
  #echo $GPGPU_CFG

  #for ((trial=1; trial<=NUM_TRIALS; trial++)); do
    #echo "Running $wl trial $trial..."
    #echo "=== TRIAL $trial ===" >> "${wl}.out_$CONFIG"

    #$ACCELSIM_OUT -trace "$KLIST" -config "$GPGPU_CFG" -config "$TRACE_CFG" >> "${wl}.out_$CONFIG"
  #done
